...
// packages/history/dist/esm/index.js
...
// packages/router-core/dist/esm/utils.js
function last(arr) {
...
// packages/router-core/dist/esm/path.js
...
// packages/router-core/dist/esm/not-found.js
...
// packages/router-core/dist/esm/qss.js
...
// packages/router-core/dist/esm/searchParams.js
...
// packages/router-core/dist/esm/root.js
var rootRouteId = "__root__";

// packages/router-core/dist/esm/redirect.js
...
// packages/router-core/dist/esm/scroll-restoration.js
...
// packages/router-core/dist/esm/router.js
  ...
  constructor(options) {
    ...
    this.buildLocation = (opts) => {
      const build = (dest = {}, matchedRoutesResult) => {
        ...
        const prevParams = __spreadValues({}, (_f = last(fromMatches)) == null ? void 0 : _f.params);
        let nextParams = (dest.params ?? true) === true ? prevParams : __spreadValues(__spreadValues({}, prevParams), functionalUpdate(dest.params, prevParams));
        if (Object.keys(nextParams).length > 0) {
          matchedRoutesResult == null ? void 0 : matchedRoutesResult.matchedRoutes.map((route) => {
            var _a2;
            return ((_a2 = route.options.params) == null ? void 0 : _a2.stringify) ?? route.options.stringifyParams;
          }).filter(Boolean).forEach((fn) => {
            nextParams = __spreadValues(__spreadValues({}, nextParams), fn(nextParams));
          });
        }
          ...
          matchedRoutesResult == null ? void 0 : matchedRoutesResult.matchedRoutes.forEach((route) => {
            try {
              if (route.options.validateSearch) {
                validatedSearch = __spreadValues(__spreadValues({}, validatedSearch), validateSearch(route.options.validateSearch, __spreadValues(__spreadValues({}, validatedSearch), search)) ?? {});
              }
            } catch {
            }
          });
          ...
          const allMiddlewares = (matchedRoutesResult == null ? void 0 : matchedRoutesResult.matchedRoutes.reduce((acc, route) => {
            ...
            if (opts._includeValidateSearch && route.options.validateSearch) {
              const validate = ({
                ...
                try {
                  const validatedSearch = __spreadValues(__spreadValues({}, result), validateSearch(route.options.validateSearch, result) ?? {});
    ...
    this.navigate = (_e) => {
      ...
      if (reloadDocument) {
        if (!href) {
          const location = this.buildLocation(__spreadValues({
            to
          }, rest));
        ...
        this.startTransition(() => __async(this, null, function* () {
          ...
          try {
            ...
            if (isResolvedRedirect(err)) {
              redirect2 = err;
              if (!this.isServer) {
                this.navigate(__spreadProps(__spreadValues({}, redirect2), {
                  replace: true,
                  ignoreBlocker: true
                }));
              }
    ...
    this.loadMatches = (_0) => __async(this, [_0], function* ({
      location,
      matches,
      preload: allPreload,
      onReady,
      ...
      let rendered = false;
      const triggerOnReady = () => __async(this, null, function* () {
        if (!rendered) {
          rendered = true;
          yield onReady == null ? void 0 : onReady();
        }
      });
      ...
      const handleRedirectAndNotFound = (match, err) => {
        ...
        if (isRedirect(err) || isNotFound(err)) {
          ...
          if (isRedirect(err)) {
            rendered = true;
            err = this.resolveRedirect(__spreadProps(__spreadValues({}, err), {
      ...
      try {
        yield new Promise((resolveAll, rejectAll) => {
          ;
            ...
            try {
              ...
              for (const [index, {
                ...
                if (
                  ...
                  yield existingMatch.beforeLoadPromise;
                ...
                if (executeBeforeLoad) {
                  try {
                    ...
                    updateMatch(matchId, (prev) => __spreadProps(__spreadValues({}, prev), {
                      ...
                      abortController,
                      pendingTimeout,
                      context: __spreadValues(__spreadValues({}, getParentMatchContext()), prev.__routeContext)
                    }));
                    const {
                      search,
                      params,
                    ...
                    const beforeLoadFnContext = {
                      ...
                      context,
                      location,
                      navigate: (opts) => this.navigate(__spreadProps(__spreadValues({}, opts), {
                        _fromLocation: location
                      })),
                    ...
                    const beforeLoadContext = (yield (_d = (_c = route.options).beforeLoad) == null ? void 0 : _d.call(_c, beforeLoadFnContext)) ?? {};
                    ...
                    updateMatch(matchId, (prev) => {
                      return __spreadProps(__spreadValues({}, prev), {
                        ...
                        context: __spreadValues(__spreadValues(__spreadValues({}, getParentMatchContext()), prev.__routeContext), beforeLoadContext),
                        abortController
                      });
                    });
                  } catch (err) {
              ...
              validResolvedMatches.forEach(({
                ...
                matchPromises.push((() => __async(this, null, function* () {
                  const {
                  ...
                  if (prevLoaderPromise) {
                    ...
                    const runLoader = () => __async(this, null, function* () {
                      var _a2, _b2, _c2, _d2, _e, _f, _g, _h, _i, _j, _k;
                      try {
                        const potentialPendingMinPromise = () => __async(this, null, function* () {
                          const latestMatch = this.getMatch(matchId);
                          if (latestMatch.minPendingPromise) {
                            yield latestMatch.minPendingPromise;
                          }
                        });
                        try {
                          ...
                          const loaderData = yield (_b2 = (_a2 = route.options).loader) == null ? void 0 : _b2.call(_a2, getLoaderContext());
                          handleRedirectAndNotFound(this.getMatch(matchId), loaderData);
                          yield route._lazyPromise;
                          yield potentialPendingMinPromise();
                          const assetContext = {
                            matches,
                        ...
                        yield route._componentsPromise;
                      } catch (err) {
                        updateMatch(matchId, (prev) => __spreadProps(__spreadValues({}, prev), {
                          loaderPromise: void 0
                        }));
    ...
    this.loadRouteChunk = (route) => {
      ...
      if (route._componentsPromise === void 0) {
        route._componentsPromise = route._lazyPromise.then(() => Promise.all(componentTypes.map((type) => __async(this, null, function* () {
    ...
    this.preloadRoute = (opts) => __async(this, null, function* () {
      ...
      try {
        matches = yield this.loadMatches({
          matches,
          location: next,
          preload: true,
        ...
        if (isRedirect(err)) {
          ...
          return yield this.preloadRoute(__spreadProps(__spreadValues({}, err), {
            _fromLocation: next
          }));
        }
        if (!isNotFound(err)) {
    ...
    this._handleNotFound = (matches, err, {
      ...
      updateMatch(matchForRoute.id, (prev) => __spreadProps(__spreadValues({}, prev), {
        status: "notFound",
        error: err,
        isFetching: false
      }));
  ...
  matchRoutesInternal(next, opts) {
    ...
    matchedRoutes.forEach((route, index) => {
      ...
      const [preMatchSearch, strictMatchSearch, searchError] = (() => {
        ...
        try {
          const strictSearch = validateSearch(route.options.validateSearch, __spreadValues({}, parentSearch)) ?? {};
          return [__spreadValues(__spreadValues({}, parentSearch), strictSearch), __spreadValues(__spreadValues({}, parentStrictSearch), strictSearch), void 0];
        } catch (err) {
    ...
    matches.forEach((match, index) => {
      ...
      if (!existingMatch && (opts == null ? void 0 : opts._buildLocation) !== true) {
        ...
        const contextFnContext = {
          ...
          location: next,
          navigate: (opts2) => this.navigate(__spreadProps(__spreadValues({}, opts2), {
            _fromLocation: next
          })),
          buildLocation: this.buildLocation,
          cause: match.cause,
...
// packages/router-core/dist/esm/defer.js
...
// packages/router-core/dist/esm/link.js
var preloadWarning = "Error preloading route! ☝️";

// packages/router-core/dist/esm/Matches.js
...
// packages/router-core/dist/esm/route.js
var BaseRoute = class {
  constructor(options) {
    this.init = (opts) => {
      var _a, _b;
      ...
      if (isRoot) {
        ...
        throw new Error(`Child Route instances must pass a 'getParentRoute: () => ParentRoute' option that returns a Route instance.`);
      }
...
var BaseRouteApi = class {
  constructor({
    ...
    this.notFound = (opts) => {
      return notFound(__spreadValues({
        routeId: this.id
      }, opts));
    };
    this.id = id;
...
// packages/router-core/dist/esm/searchMiddleware.js
function retainSearchParams(keys) {
  return ({
    ...
    if (keys === true) {
      return __spreadValues(__spreadValues({}, search), result);
    }
    keys.forEach((key) => {
      if (!(key in result)) {
...
//# sourceMappingURL=chunk-3JUTRO22.js.map
